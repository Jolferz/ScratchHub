...
//         profile DELETE          //
// =============================== //
router.delete('/:user/profile-delete', (req, res) => {
  ...
  .exec((err, user) => {
    ...
    Project.find({ author: user }, (err, projects) => {
      if (err) return err
      // remove every single project
      for (let i = 0; i < projects.length; i++) {
        // remove project
        Project.remove({_id: projects[i]}, (err, rmProjects) => {
          if (err) return err
          // delete project's image
          fs.unlink('uploads/' + projects[i].name + '.png', (cb) => {
